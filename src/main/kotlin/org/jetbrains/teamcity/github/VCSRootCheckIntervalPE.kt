...
public class VCSRootCheckIntervalPE(descriptor: PluginDescriptor, places: PagePlaces) : HealthStatusItemPageExtension(VCSRootCheckIntervalHR.TYPE, places) {
    @Autowired
    lateinit var helper: TokensHelper
    private val myResourcesPath = descriptor.pluginResourcesPath

    init {
        includeUrl = descriptor.getPluginResourcesPath("vcs-root-health-item.jsp")
    ...
    override fun fillModel(model: MutableMap<String, Any>, request: HttpServletRequest) {
        ...
        val root = item.additionalData["VcsRoot"] as SVcsRoot
        val info = item.additionalData["GitHubInfo"] as VcsRootGitHubInfo
        val hook = item.additionalData["HookInfo"] as WebHooksStorage.HookInfo
